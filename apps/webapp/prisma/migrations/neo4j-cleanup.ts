...
export async function cleanupNeo4jSpaces() {
  ...
  try {
    ...
    console.log(
      `✓ Deleted ${deletedSpaces} Space nodes and all their relationships\n`,
    );
    ...
    console.log(
      `✓ Cleaned ${episodeCount} Episode nodes (removed spaceIds and space fields)\n`,
    );
    ...
    console.log(
      `✓ Cleaned ${statementCount} Statement nodes (removed spaceIds and space fields)\n`,
    );
    ...
    const episodesWithSpaceIds =
      record?.get("episodesWithSpaceIds").toNumber() || 0;
    const episodesWithSpace = record?.get("episodesWithSpace").toNumber() || 0;
    const statementsWithSpaceIds =
      record?.get("statementsWithSpaceIds").toNumber() || 0;
    const statementsWithSpace =
      record?.get("statementsWithSpace").toNumber() || 0;

    console.log(`✓ Remaining Space nodes: ${remainingSpaces} (should be 0)`);
    console.log(
      `✓ Episodes with spaceIds: ${episodesWithSpaceIds} (should be 0)`,
    );
    console.log(`✓ Episodes with space: ${episodesWithSpace} (should be 0)`);
    console.log(
      `✓ Statements with spaceIds: ${statementsWithSpaceIds} (should be 0)`,
    );
    console.log(
      `✓ Statements with space: ${statementsWithSpace} (should be 0)`,
    );

    if (
      remainingSpaces > 0 ||
      episodesWithSpaceIds > 0 ||
      episodesWithSpace > 0 ||
      statementsWithSpaceIds > 0 ||
      statementsWithSpace > 0
    ) {
      throw new Error(
        "❌ Cleanup verification failed: Some space data still exists",
      );
    }
